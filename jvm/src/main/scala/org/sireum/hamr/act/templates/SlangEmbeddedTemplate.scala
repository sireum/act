...
object SlangEmbeddedTemplate {
  ...
  def hamrIsEmptyUnconnected(methodName: String,
    ...
    val ret: ST =
      st"""// FIXME: dummy implementation for unconnected incoming port
  ...
  def hamrIsEmpty(methodName: String,
    ...
    val ret: ST =
      st"""// is_empty ${CommonUtil.getLastName(srcPort.identifier)}: ${srcPort.direction.name} ${srcPort.category.name}
          |B ${methodName}(${StackFrameTemplate.STACK_FRAME_ONLY}) {
  ...
  def hamrSendUnconnectedOutgoingDataPort(methodName: String,
                                          declNewStackFrame: ST): ST = {
    val ret: ST =
      st"""// FIXME: dummy implementation for unconnected outgoing port
          |Unit ${methodName}(
          |  ${StackFrameTemplate.STACK_FRAME}
          |  art_DataContent d) {
          |  ${declNewStackFrame};
